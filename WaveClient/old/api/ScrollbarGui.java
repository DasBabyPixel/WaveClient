...
import de.dasbabypixel.waveclient.wrappedloader.api.render.ScalingManager.Scale;
...
public class ScrollbarGui extends AbstractGui implements GuiWrapper {
	...
	public void draw(double mouseX, double mouseY) {
		final double scale = WaveClientAPI.getInstance().getScalingManager().getScale();
		WaveClientAPI.getInstance().getScalingManager().pushScale(new Scale(1, Scale.Type.OVERRIDE));
...
//		gui.onScroll(delta);
	...
	public double getWidth() {
		return verticalScrollbar.isVisible()
				? getWidthWithScrollbar(WaveClientAPI.getInstance().getScalingManager().getScale())
	...
	public double getHeight() {
		return horizontalScrollbar.isVisible()
				? getHeightWithScrollbar(WaveClientAPI.getInstance().getScalingManager().getScale())
	...
	public final class Scrollbar {
		...
		public void draw(double x, double y, double width, double height, double mouseX, double mouseY, double bgx,
			...
			if (height != 0) {
				...
				DrawHelper.drawRect(bgx, bgy, bgWidth, bgHeight, scrollbarBackground);
				DrawHelper.drawRect(x, y, width, height, hovering || modifying ? scrollbarHoverColor : scrollbarColor);
			}
		}
